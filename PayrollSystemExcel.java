...
public class PayrollSystemExcel {
    ...
    // Attendance: each entry = {employeeNumber, month, dayOfMonth, hoursWorked}
    ...
    static class MonthlyPayroll {
        int month;
        int lastDay;
        CutoffPayroll firstCutoff;
        CutoffPayroll secondCutoff;
        double sss;
        double philHealth;
        double pagIbig;
        double tax;
    ...
    static CutoffPayroll buildCutoffPayroll(int empNo, int month, int startDay, int endDay, double hourlyRate) {
        CutoffPayroll cutoff = new CutoffPayroll();
        cutoff.hoursWorked = computeTotalHours(empNo, month, startDay, endDay);
        cutoff.grossSalary = cutoff.hoursWorked * hourlyRate;
        cutoff.netSalary = cutoff.grossSalary;
        return cutoff;
    }

    static MonthlyPayroll buildMonthlyPayroll(int empNo, int month, double hourlyRate) {
        MonthlyPayroll payroll = new MonthlyPayroll();
        payroll.month = month;
        payroll.lastDay = lastDays[month];

        payroll.firstCutoff = buildCutoffPayroll(empNo, month, 1, 15, hourlyRate);
        payroll.secondCutoff = buildCutoffPayroll(empNo, month, 16, payroll.lastDay, hourlyRate);

        double monthlyGross = payroll.firstCutoff.grossSalary + payroll.secondCutoff.grossSalary;
        payroll.sss = computeSSS(monthlyGross);
        payroll.philHealth = computePhilHealth(monthlyGross);
        payroll.pagIbig = computePagIBIG(monthlyGross);
        double taxableIncome = monthlyGross - payroll.sss - payroll.philHealth - payroll.pagIbig;
        payroll.tax = computeTax(taxableIncome);
        payroll.totalDeductions = payroll.sss + payroll.philHealth + payroll.pagIbig + payroll.tax;
        payroll.secondCutoff.netSalary = payroll.secondCutoff.grossSalary - payroll.totalDeductions;

        return payroll;
    }

    static void printCutoffHeader(String label) {
        System.out.println("================================================");
        System.out.println(label);
        System.out.println("================================================");
    }

    static void printEmployeeDetails(int empNo, String name, String birthday) {
        System.out.printf("Employee #:          %d%n", empNo);
        System.out.printf("Employee Name:       %s%n", name);
        System.out.printf("Birthday:            %s%n", birthday);
    }

    static void printCutoffTotals(double totalHours, double grossSalary, double netSalary) {
        System.out.printf("Total Hours Worked:  %f%n", totalHours);
        System.out.printf("Gross Salary:        PHP %,f%n", grossSalary);
        System.out.printf("Net Salary:          PHP %,f%n", netSalary);
        System.out.println();
    }

    static void printDeductions(double sss, double philHealth, double pagIbig, double tax, double totalDeductions) {
    ...
    static void displayPayroll(int index) {
        ...
        for (int month : months) {
            MonthlyPayroll payroll = buildMonthlyPayroll(empNo, month, hourlyRate);

            // Cutoff 1 display
            printCutoffHeader(String.format("Cutoff Date: %s 1 to %s 15", monthNames[payroll.month], monthNames[payroll.month]));
            printEmployeeDetails(empNo, name, birthday);
            printCutoffTotals(payroll.firstCutoff.hoursWorked, payroll.firstCutoff.grossSalary, payroll.firstCutoff.netSalary);

            // Cutoff 2 display
            printCutoffHeader(String.format("Cutoff Date: %s 16 to %s %d", monthNames[payroll.month], monthNames[payroll.month], payroll.lastDay));
            printCutoffTotals(payroll.secondCutoff.hoursWorked, payroll.secondCutoff.grossSalary, payroll.secondCutoff.grossSalary);
            printDeductions(payroll.sss, payroll.philHealth, payroll.pagIbig, payroll.tax, payroll.totalDeductions);
            System.out.printf("Net Salary:          PHP %,f%n", payroll.secondCutoff.netSalary);
            System.out.println("================================================");
            System.out.println();
        }
    }

    static void runEmployeePortal(Scanner scanner) {
    ...
    static void runPayrollStaffPortal(Scanner scanner) {
        boolean running = true;
        while (running) {
            System.out.println();
            System.out.println("========================================");
    ...
    public static void main(String[] args) {
        ...
        // --- LOAD EXCEL FILE ---
        String excelFile = "MotorPH_Employee Data.xlsx";
        ...
        if (username.equals("employee")) {
            runEmployeePortal(scanner);
        }

        if (username.equals("payroll_staff")) {
            runPayrollStaffPortal(scanner);
        }
