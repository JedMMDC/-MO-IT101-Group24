...
public class PayrollSystemExcel {
    ...
    // Returns {hoursWorked, grossSalary, netSalary}
    static double[] computeCutoffPayroll(int empNo, int month, int startDay, int endDay, double hourlyRate) {
        double hoursWorked = computeTotalHours(empNo, month, startDay, endDay);
        double grossSalary = hoursWorked * hourlyRate;
        double netSalary = grossSalary;
        return new double[]{hoursWorked, grossSalary, netSalary};
    }
    ...
    static void displayPayroll(int index) {
        ...
        for (int month : months) {
            ...
            double[] firstCutoff = computeCutoffPayroll(empNo, month, 1, 15, hourlyRate);
            double[] secondCutoff = computeCutoffPayroll(empNo, month, 16, lastDay, hourlyRate);

            double monthlyGross = firstCutoff[1] + secondCutoff[1];
            ...
            double secondCutoffNetSalary = secondCutoff[1] - totalDeductions;

            // Cutoff 1 display
            printCutoffHeader(String.format("Cutoff Date: %s 1 to %s 15", monthNames[month], monthNames[month]));
            printEmployeeDetails(empNo, name, birthday);
            printCutoffTotals(firstCutoff[0], firstCutoff[1], firstCutoff[2]);

            // Cutoff 2 display
            printCutoffHeader(String.format("Cutoff Date: %s 16 to %s %d", monthNames[month], monthNames[month], lastDay));
            printCutoffTotals(secondCutoff[0], secondCutoff[1], secondCutoff[1]);
            printDeductions(sss, philHealth, pagIbig, tax, totalDeductions);
            System.out.printf("Net Salary:          PHP %,f%n", secondCutoffNetSalary);
            System.out.println("================================================");
            System.out.println();
        }
    }
